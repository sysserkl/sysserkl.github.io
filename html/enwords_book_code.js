    ...
    switch (cstype){
        ...
        case 'kaikki phrase':
            if (check_kaikki_phrase_b()){
                ...
                var blstr=(csmax>0?kaikki_phrase_global.slice(0,csmax):kaikki_phrase_global).join('\n');
                if (cstype=='kaikki phrase'){
...
function day_new_enwords_book(do_filter=false,group_by='y'){
    document.getElementById('textarea_new_words1').value=day_new_enbook_b(-1,do_filter,group_by).join(' ');    
}
