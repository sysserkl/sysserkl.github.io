...
function input_range_sd(){
    var bljg = "";
    bljg='<input type="range" min=1 max='+(sudoku_data_global.length+1)+' value=1 id="sudoku_range" style="width:20rem;" oninput="javascript:document.getElementById(\'a_no\').innerHTML=\'第 \'+this.value+\' 题\';" onchange="javascript:show_sd(this.value-1);"> ';
    bljg=bljg+'<a href="javascript:void(null);" id="a_no" onclick="javascript:show_sd(parseInt(this.innerText.split(\' \')[1])-1);">第 1 题</a>';
...
function show_sd(csnumber,csreform=false){
	...
	for (var blxl in blarray){
		...
		bltmp = blarray[blxl];
		if (bltmp == "0") {bltmp = empty_sd_global;}
        blstyle='';
        ...
        if (blarray[blxl]!=="0"){
            blstr = blstr + '<td class="td_sudoku" width=1 id="input_'+blxl+'" nowrap align=center style="'+blstyle+'background-color:#e0e0e0;" onclick="javascript:td_background_sd(this.id,\'onlysamenum\');">'+bltmp+'</td>';
        }
        else {
		    blstr = blstr + '<td class="td_sudoku"  width=1 id="input_'+blxl+'" nowrap align=center style="'+blstyle+'color:blue;" onclick="javascript:td_background_sd(this.id);">'+bltmp+'</td>';
        }
		if (blxl%9 == 8){blstr = blstr + '</tr>'}
	}
    blstr=blstr+'<tr><td colspan=9 align=center style="border:0;font-size:'+(font_size_sd_global-0.5)+'rem;">';
    blstr=blstr + "<a href=\"javascript:void(null);\" onclick=\"javascript:getvalue_check_sd()\">检查</a> ";
    blstr=blstr + "<a href=\"javascript:void(null);\" onclick=\"javascript:show_sd("+csnumber+",true)\">题目变形</a> ";
    blstr=blstr + "<a href=\"javascript:void(null);\" onclick=\"javascript:fill_sd()\">答案</a>";
    blstr=blstr+'</td></tr>';
    ...
    //录入条 - 保留注释
    blstr=blstr+'<td align=center onclick="javascript:inputvalue_sd(empty_sd_global);">↪️</td>';
    for (var blxl=1;blxl<=9;blxl++){
        blstr=blstr+'<td align=center onclick="javascript:inputvalue_sd(this.innerHTML);" id="td_inputvalue_sd'+blxl+'">'+blxl+'</td>';
    }
...
function style_sd(){
    document.write('\n<style>\n');
    //document.write("h2{margin:0rem;font-size:"+(font_size_sd_global-1)+"rem;}");
    document.write(".table_sudoku td{font-weight:600;border:0.1rem #e0e0e0 solid;padding:0rem 0.95rem;font-size:"+font_size_sd_global+"rem;cursor:pointer;}");
    document.write(".table_select td{font-weight:600;border:0;padding:0rem 0.5rem;font-size:"+(font_size_sd_global+0.3)+"rem;color:tomato;background-color:pink;cursor:pointer;}");
    document.write('#a_no{font-size:'+(font_size_sd_global-0.5)+'rem;border:0.2rem solid #c0c0c0;border-radius: 1rem; padding:0.1rem 0.5rem;margin:0rem 0.1rem;text-decoration:none;}');
    document.write('\n</style>\n');
}
