function import_data_klsda(){
    ...
    document.write('\n<SCRIPT language=JavaScript src="../jsdata/sda/'+data_type+'_data.js"><\/SCRIPT>\n');
...
function menu_klsda(){
    ...
    var klmenu1=[
    '<a href="http://tjj.zj.gov.cn/" onclick="javascript:'+str_t+'" target=_blank>浙江省统计局</a>',    
    '<a href="http://data.tjj.zj.gov.cn/" onclick="javascript:'+str_t+'" target=_blank>数据浙江</a>',    
    ];
    ...
    var klmenu_config=[
    '<span class="span_menu" onclick="javascript:'+str_t+'batch_lines_klsda();">折线图批量</span>',
    ];
    
    klmenu_config=klmenu_config.concat(root_font_size_menu_b(str_t));
    
    document.getElementById('h2_title').insertAdjacentHTML('afterbegin',klmenu_multi_button_div_b(klmenu_b(klmenu1,'','10rem','1rem','1rem','60rem')+klmenu_b(menu_dbf,'🛢','14rem','1rem','1rem','60rem')+klmenu_b(klmenu_config,'⚙','10rem','1rem','1rem','60rem'),'','0rem')+' ');
}
...
function draw_lines_klsda(){
    ...
    document.getElementById("div_flot").style.cssText="width:100%;height:72%;margin-bottom:30px;";
    ...
    if (split_no==0){
        ...
        flot_two_lines_two_yaxis_k(list_t,'div_flot',unit1,unit2,'nw',false,'m',2,2,'',[],-1,false,false,false,false,list_t.length-split_no);
    }
}
...
function draw_pie_klsda(){
    ...
    document.getElementById("div_flot").style.cssText="width:100%;height:90%;margin:30px;";
    flot_pie_k(chart_data,'div_flot');
}
...
function raw_data_check_klsda(){
    ...
    console.log(Array.from(category_set).sort());
    ...
    for (let ayear of year_list){
        ...
        for (let arow=1;arow<one_year_data.length;arow++){//忽略第一行 城市名称 - 保留注释
            ...
            for (let acol=2;acol<one_year_data[arow].length;acol++){//忽略前2列（指标名称 单位） - 保留注释
                ...
                if (result_t[city_name+'_'+category_name]==null){
