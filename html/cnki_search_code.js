...
function menu_cnki(){
    ...
    var klmenu_pb_data=[];
    var pb_list=['ChatGPT','Python','共同富裕','良渚','三次分配','三星堆','社会主义核心价值观'];
    for (let item of pb_list){
        klmenu_pb_data.push('<span class="span_menu" onclick="'+str_t+'import_cnki(this.innerText,\'pb\');">'+item+'</span>');
    }
    
    var klmenu_kl_data=[];
    for (let item of cnki_kl_list_global){
        klmenu_kl_data.push('<span class="span_menu" onclick="'+str_t+'import_cnki(this.innerText,\'kl\');">'+item+'</span>');    
    }
    ...
    '<span id="span_reg_cnki" class="span_menu" onclick="'+str_t+'klmenu_check_b(this.id,true);">⚪ reg</span>',   
    '<span id="span_highlight_cnki" class="span_menu" onclick="'+str_t+'klmenu_check_b(this.id,true);">⚪ highlight</span>',    
    '<span class="span_menu" onclick="'+str_t+'standalone_cnki();">当前结果导出为 standalone search</span>',    
    '<span class="span_menu" onclick="'+str_t+'slash_cnki();">当前结果生成为 /// 格式</span>',    
...
function import_cnki(cskey,cstype){
...
function table_th_cnki(is_simple=false){
    var th_names=['题名','作者','来源','发表时间','数据库','被引','下载'];
    var th_list=[];
    if (is_simple){
