...
function show_symbols(cstype='simple'){
    var bljg='';
    var divstyle_global='display:inline-block;padding:0.2rem;margin:0.2rem;border:0.1rem grey dotted;border-radius:0.5rem;';
    for (let blkey in symbols_global){
        ...
        if (cstype=='simple' && symbols_global[blkey].length>8){
            ...
            bljg=bljg+' <span id="span_'+blkey+'" style="cursor:pointer;" onclick="javascript:more_symbols(this)">...</span> ';
...
function menu_symbols(){
    ...
    var klmenu1=[
    '<span class="span_menu" onclick="javascript:'+str_t+'show_symbols(\'all\');">全部展开</span>',
    ];
