...
function compare_diff_b(do_submit=false){
    ...
    if (diff1.length>0){
        ...
        result_t.push(array_2_li_b(diff1));
    }
    if (diff2.length>0){
        result_t.push('<h2 style="background-color:'+scheme_global['pink']+';">+</h3>');
        result_t.push(array_2_li_b(diff2));
    }
...
function buttons_diff_b(){
    ...
    bljg=bljg+'<span class="aclick" onclick="tab2space_diff_b();">替换连续空格为一个空格，替换\\t为空格</span> ';
    bljg=bljg+'<span class="aclick" onclick="remove_empty_lines_diff_b();">去除空行</span> ';
    bljg=bljg+'<span class="aclick" onclick="eng_punctuatin_diff_b();">转换为英文标点</span> ';
    bljg=bljg+'<span class="aclick" onclick="remove_all_space_diff_b();">移除所有空格</span> ';
    bljg=bljg+'<span class="aclick" onclick="remove_cn_space_diff_b();">移除汉字间的空格</span> ';    
    bljg=bljg+'<span class="aclick" onclick="hide_show_diff_b();">隐藏/显示相同行</span> ';
    bljg=bljg+'<span class="aclick" onclick="clear_textarea_bottle(\'12\',\'diff\');">清空两个编辑框</span> ';
    bljg=bljg+'<span class="aclick" onclick="compare_diff_b();">行差集</span> ';    
    
    document.getElementById('span_diff_buttons').innerHTML=bljg;
    
    document.querySelector('textarea[name="textarea_diff_1"]').insertAdjacentHTML('beforebegin','<p><span class="aclick" onclick="clear_textarea_bottle(\'1\',\'diff\');">清空</span> <input type="file" id="input_data_upload_txt1"> <span class="aclick" onclick="upload_txt_diff_b(1);">Upload TXT File</span></p>');
    document.querySelector('textarea[name="textarea_diff_2"]').insertAdjacentHTML('beforebegin','<p><span class="aclick" onclick="clear_textarea_bottle(\'2\',\'diff\');">清空</span> <input type="file" id="input_data_upload_txt2"> <span class="aclick" onclick="upload_txt_diff_b(2);">Upload TXT File</span></p>');
