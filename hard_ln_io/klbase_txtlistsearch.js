...
function txtmenus_kltxt_b(cstype=''){
    ...
    enwords_key_new_rare_menu_kltxt_b(menu_general,str_t,true);
...
function recent_added_in_digest_kltxt_b(){
    ...
    if (confirm('是否返回reg格式？')){
        ...
        if (oinput){
            oinput.value='+kleng +'+enwords_list_2_reg_b(csarr);
            oinput.focus();
        } else {
...
function current_page_rare_words_kltxt_b(cstype='',textarea_mode=false){
    ...
    if (textarea_mode){
        document.getElementById('divhtml').innerHTML=new_old_words_html_enbook_b(new Set(key_list),caption,'',false,new Set(),true,true);
    }
    return key_list;
}
...
function ensentence_in_textarea_remove_open_end_b(){
    ...
    var result_t=[];
    for (let arow of list_t){
        if (arow.trim().match(open_end_key)){continue;}
        result_t.push(arow);
    }
    otextarea.value=result_t.join('\n\n');
    ensentence_in_textarea_len_b(result_t);
...
function ensentence_in_textarea_group_b(){
    ...
    for (let blxl=0,lent=rare_dict.length;blxl<lent;blxl++){
        ...
        if (arow.length<blminor){
            console.log(rare_dict.length);        
            right_part_list=rare_dict.slice(blxl,);
            rare_dict=rare_dict.slice(0,blxl);
            console.log(right_part_list.length,rare_dict.length);
            break;
        }
    ...
    var merge_list=[];
    for (let arow of right_part_list){
        merge_list=merge_list.concat(arow.slice(1,));
    }
    merge_list='<h4>其他</h4>'+ensentence_in_sub_textarea_b(merge_list.join('\n\n'),merge_list.length,blno+1);
