...
function open_link_en_b(cstype,csword){
    switch (cstype){
        ...
        case '4':
            ...
            break;
        case 'k':
            window.open('PythonTools/data/selenium_news/html/klsearch.htm?k='+encodeURIComponent(csword)+'&t=dict.cn,bing(cn),merriam-webster,cambridge,collins(p),youdao,iciba,cambridge_cn,wordreference_cn,wikitionary(p)&iframe');            
    }
}
...
function popup_words_links_b(event,csword){
    var bljg='<span style="cursor:pointer;font-weight:bold;'+(en_words_temp_important_global.includes(csword)?'color:'+scheme_global["a-hover"]+';':'')+'" onclick="javascript:en_word_temp_important_change_b(this);">'+csword+'</span> ';
    bljg=bljg+'<br />'+en_word_links_b(csword,true);
    popup_event_div_b(event,'div_enword_search_links',bljg,'bottom');
}
...
function en_words_temp_textarea_b(divname,calljsname=''){
    ...
    bljg=bljg+'<textarea name="textarea_word_temp" id="textarea_word_temp" style="height:'+(ismobile_b()?'10':'20')+'rem;">'+en_word_temp_get_b(true)+'</textarea>';
    bljg=bljg+'<p>';
...
function en_word_temp_get_b(is_raw=false){
    if (is_raw){
