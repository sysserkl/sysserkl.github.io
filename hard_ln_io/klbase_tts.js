...
function init_temp_txt_share_b(is_php=true,menu_more='',do_add=true,is_simple=false,buttons_more=''){
    ...
    document.getElementById('span_update').insertAdjacentHTML('beforebegin',buttons+textarea_buttons_b('textarea_temp_txt_share','清空,复制,↑,↓,加密,解密,行数,save as txt file,导入 txt 文件'+(is_php?'':',发送到临时记事本,发送地址,➕')));
    ...
    if (otextarea){
        otextarea.style.height=(ismobile_b()?8:20)+'rem';
    }
}
